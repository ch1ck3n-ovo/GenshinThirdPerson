...
public class GenshinCamera {
    ...
    public boolean canRotate() {
        ...
        GTPConfig.CameraAlignWhileClick cameraAlignWhileClick = instance.getConfig().cameraAlignWhileClick;
        GTPConfig.CameraBasedMovement cameraBasedMovement = instance.getConfig().cameraBasedMovement;
        if (cameraAlignWhileClick.status && System.currentTimeMillis() - this.getLastInteractionTimeMillis() <= cameraAlignWhileClick.alignRecoveryDelay * 50L) {
            return false;
        }
        if (cameraBasedMovement.disableWhenElytra && player.isGliding()) {
            return false;
        }
        if (cameraBasedMovement.disableWhenRiding && player.hasVehicle()) {
            return false;
        }
